...
int MainFrameWindow::LoadToolBoxPlugins(QString strConfigFile)
{
	...
	if(file.open(QFile::ReadOnly))
	{
		...
		nodeList = XmlReader.elementsByTagName(QString::fromLocal8Bit("StatusBar"));
		if(nodeList.size() == 1)
		{
			LoadStatustoolbar(&nodeList.at(0));
		}
...
nephogram_ground::Map* MainFrameWindow::GetCurrentMap()
{
...
void MainFrameWindow::slot_subWindowActivated( QMdiSubWindow * window )
{
	...
	QVariant qVar = pWidget->property("nephogram_ground::Map");
	ULONGLONG ullMapPointer = qVar.toULongLong();
	pcfc->SetCurrentMap((nephogram_ground::Map*)ullMapPointer);
...
void MainFrameWindow::LoadStatustoolbar( QDomNode* pDN )
{
