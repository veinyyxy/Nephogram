...
DrawGeometryManipulator* SwitchCameraMainpulator()
{
	...
	nephogram_ground::ManualDrawLayer* pLayer = new nephogram_ground::ManualDrawLayer(0, 0, 1);
	pLayer->LayerZ(20);
	...
	nephogram_ground::Map* pMap = (nephogram_ground::Map*)GetViwer()->property("Map").toLongLong();//Map2D();
	nephogram_ground::Layer* poldLayer = pMap->GetLayer("·ûºÅ²ã");
	if(!poldLayer)
...
void DrawPolygonalLineAction::MyAction()
{
	...
	nephogram_ground::Map* pMap = (nephogram_ground::Map*)GetViwer()->property("Map").toLongLong();;
