...
public:
	...
	{
		dp = da;
		m_Proj4 = new nephogram_ground::Proj4Transform;
	}
	virtual bool handle( const GUIEventAdapter& ea,GUIActionAdapter& aa, osg::Object*, osg::NodeVisitor* )
	{
		...
		QString val = QString::number(value);// + dp->GetDataUnit();//2020
		m_pMap = dp->ParentLayer()->GetParentMap();
		m_pMap->SetCurrentValue(val.toStdString());
		return osgGA::GUIEventHandler::handle(ea,aa);
	}
	nephogram_ground::Proj4Transform* m_Proj4;
	DataProvider *dp;
	Map* m_pMap;
...
QWidget* RadarDataAccess::ReadRadarData( const QString& filename, int wdt,QWidget* parent /*= 0*/)
{
	...
	if (dm)
	{
		...
		if (pCircleLayer != 0)
		{
			...
			nephogram_ground::CoordinateTransform* pCT = extMap->GetCoordinateTransform();
...
void RadarDataAccess::LoadRadarData( QString filename ,Map* map)
{
	...
	nephogram_ground::Ground* pGeosOnTheosg = nephogram_ground::Ground::Instantiate();
	
	nephogram_ground::VectorLayer* vLayer0 = new nephogram_ground::VectorLayer();
	vLayer0->SetParentMap(map);
	vLayer0->Visible(true);
	vLayer0->CoordTrans(map->GetCoordinateTransform());
	pGeosOnTheosg->OpenDataAsLayer(filename.toStdString(), vLayer0);
	//////////////////////////////////////////////////////////////////////////
...
void RadarDataAccess::StartShowLayerIndex(Map* map , int index)
{
	...
	std::vector<std::string> blist = map->BasisLayerList();
	while(layerIter != m_mapLayer.end())
	{
		if (blist.(layerIter->first))
		{
			layerIter++;
			continue;
		}
...
void RadarDataAccess::PlotWidget( ViewerQT* viewer )
{
	...
	Map* map = (nephogram_ground::Map*)viewer->property("Map").toLongLong();;
...
QVariant RadarDataAccess::GetAttributeValue(int index ,Layer* layer)
{
	/*boost::any _value;
...
void RadarDataAccess::VolumeRender( Map* srcMap )
{
		...
		nephogram_ground::Map* map1 = pMap;

		nephogram_ground::Layer* pCircleLayer = map1->GetLayer("距离圈");
		...
		if (pCircleLayer != 0)
		{
			...
			nephogram_ground::Layer* pRadarLayer = map1->GetLayer("第1仰角层");
			if(!pRadarLayer) return;
			nephogram_ground::DataProvider* pDP = pRadarLayer->GetDataProvider();
			if(!pDP) return;
...
{
	nephogram_ground::Proj4Transform* coord = new nephogram_ground::Proj4Transform;
	...
	//nephogram_ground::RadarVectorLayer* vLayer0 = (nephogram_ground::RadarVectorLayer*)(pGeosOnTheosg->OpenRadarDataAsLayer(filename.toStdString(),map,coord));
	//////////////////////////////////////////////////////////////////////////
