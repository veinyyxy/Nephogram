...
{
	m_p2DBasicMap = Ground::Instantiate()->GetMapFactory()->CreateMap();
...
QWidget* ViewManager::CreateViewport(int wdt ,  bool basicMap/* = true*/ ,QWidget* parentWidget)
{
	...
	else
	{
		...
		ViewerQT*  viewerWindow = new ViewerQT(pViewer,parentWidget);
		//viewerWindow->setViewer(pViewer);
...
#if 0
boost::any ViewManager::GetAttributeValue(int index ,nephogram_ground::Layer* layer)
{
	boost::any _value;
