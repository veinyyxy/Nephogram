...
ClipCallback::ClipCallback( nephogram_ground::Map* map )
{
	_map = map;
}


void ClipCallback::DrawPlotGraph(nephogram_ground::Map* srcMap, osg::Vec3& p1, osg::Vec3& p2, osg::Vec3& p3, osg::Vec3& p4, osg::Node* pClipNode )
{
	...
	if (dataProvider)
	{
		...
		//pVI->Cutting(p1,p2,p3,p4,pClipNode,srcMap->GetCoordinateTransform());//2020
		return;
	}
}
...
void ClipCallback::DrawPlotGraph(nephogram_ground::Map* srcMap, QImage* pImage, osg::Vec3& p1, osg::Vec3& p2, osg::Vec3& p3, osg::Vec3& p4)
{
	...
	if (dataProvider)
	{
		...
		InterpolationRadarDataInterface* pRadarInterface = (InterpolationRadarDataInterface*)(pvoid);
		if(pRadarInterface)
		{
			...
			pRadarInterface->SetStartEnd(&_x1,  &_y1,  &_x2,  &_y2/*,dRatio,ProductCenter_X,ProductCenter_Y*/);//2020
			...
			while(layerIter != layerAttributeDescripe->end())
			{
				...
				QVariant _any = GetAttributeValue(index,pMap->GetLayer("第1仰角层"));
				if (typeName == "String") // 字符串
				{
					std::string _value = _any.toString().toStdString();
			...
			pPlotDialog.GetImage(pImage);*///2020
		}
	}
}