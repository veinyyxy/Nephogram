...
{
	Ground* pGeosOsg = Ground::Instantiate();
	...
	if(file.open(QFile::ReadOnly))
	{
		...
		for (int i = 0; i < BasisLayer.length(); i++)
		{
			layerList.push_back(BasisLayer.at(i).toStdString());
		}
		pMap->BasisLayerList(layerList);
...
Layer* GISMapManager::NewRasterLayer(const QString& fileName, qreal qrz )
{
	...
	nephogram_ground::CoordinateTransform* ct = m_pMap->GetCoordinateTransform();
	...
	nephogram_ground::Layer* pLayer = new nephogram_ground::VectorLayer(pDataRovider, ct);
	pLayer->LayerZ(qrz);
