...
public:
	...
	virtual void operator()(osg::Node* node, osg::NodeVisitor* nv)
	{
		...
		if(pMT && m_pSRS && m_pViewer)
		{
			...
			//new osgearth 2020
			double dAngle = viewPoint.pitch()->getValue() + 90;
...
void OsgEarthBridge::LoadSky()
{
	...
	if (m_EarthMapNode)
	{
		...
		osgEarth::Util::StringSet as;
		m_EarthMapNode->getMap()->getAttributions(as);
		if(as.find("Geocentric") != as.end())
		{
			// Sky model.
			osgEarth::Config skyConf = externals.child("sky");

			double hours = skyConf.value("hours", 13);
			s_sky = osgEarth::SkyNode::create();
...
#if 1
osg::Node* createLine(osg::Vec3& p1, osg::Vec3& p2, osg::Vec4& colors)
{
