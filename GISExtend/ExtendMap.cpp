...
{
public:
	ViewerEventHandle(){m_Proj4 = new nephogram_ground::Proj4Transform;}
	...
	m_2DMapManager(this)
{
	...
	std::string name1("IsoSurface");
	std::string name2("ClipPlaneInterface");
	SetExtendInterface(name1, (IsoSurface*)(&m_3DMapManager));
	SetExtendInterface(name2, (ClipPlaneInterface*)(&m_3DMapManager));
}
...
void ExtendMap::MapLocation( float fLon, float fLat )
{
	...
	nephogram_ground::CoordinateTransform* pCT = GetCoordinateTransform();
...
void ExtendMap::Site( osg::Vec3 pos )
{
	...
	osgEarth::Util::Viewpoint vp("myview", pos.y(), pos.x(), 0, 45, 0, 100000.0);
