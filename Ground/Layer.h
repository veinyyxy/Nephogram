...
class GROUND_EXPORT Layer
{
public:
	typedef std::map<int, std::auto_ptr<Geometry>> MapGeometry, *P_MapGeometry;
	typedef std::pair<int, std::auto_ptr<Geometry>> MapGeometryPair, *P_MapGeometryPair;
...
public:
	...
	inline MapGeometry* GetMgoGeometry() {return m_pgoGeometry.get();}
...
protected:
	...
	std::auto_ptr<DataProvider> m_dataProvider;
	//是否参加投影变换（针对雷达单站数据）
	bool b_trans;
	bool m_bUseLayerZ;
	unsigned int m_textColor;
	std::auto_ptr<MapGeometry> m_pgoGeometry;
	float m_fontSize;
