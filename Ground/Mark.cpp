...
Mark::Mark(void) : m_bUsedSet(true)
{
...
Mark::Mark( std::string markLabel,osg::Vec3 markPostion , Layer* pL): m_bUsedSet(true)
{
	m_markText = new osgText::Text();
...
void Mark::CreateOsgFromGeos( osg::Node* pNode )
{
	...
	osg::Vec3 destVec3;
	if (m_pCoordinateTransform == 0)
	{
		m_pCoordinateTransform =m_pParentLayer->CoordTrans();//GetParentMap()->GetCoordinateTransform();
	}

	if(!m_pCoordinateTransform)
	{
	...
	m_pCoordinateTransform->Transform(m_markPostion, destVec3);
...
void Mark::SetCoordinateTransform( CoordinateTransform* pCT )
{
	m_pCoordinateTransform = pCT;
}

geom::GeometryTypeId Mark::GetGeometryType()
{
		return (geom::GeometryTypeId)100;
}

void Mark::TransformGeometry()
{
	osg::Vec3 destVec3;
	if (m_pCoordinateTransform == 0)
	{
		m_pCoordinateTransform = m_pParentLayer->CoordTrans();//->GetCoordinateTransform();
	}
...
void Mark::operator=( const Mark& other )
{
