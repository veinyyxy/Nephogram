...
void Map::AddFishNet( float lonstart,float latstart,float lonend,float latend,float lonCellSize,float LatCellSize , double qrz )
{
...
int Map::RemoveLayer( Layer* pLayer, bool bIsDel /*= true*/ )
{
...
void Map::operator=( const Map& other )
{
	...
	while(layerIter != mapLayers.end())
	{
		...
		Layer* thisLayer = new VectorLayer(other.m_pDataProvider,this->m_pCoordTrans);
