...
public:
	...
	inline std::vector<std::string>& BasisLayerList() {return m_basisLayerList;}
	inline void BasisLayerList(std::vector<std::string> basisLayerList){m_basisLayerList = basisLayerList;}
	void TramsformMap();
	...
	int RemoveLayer(Layer* pLayer, bool bIsDel = true);
	...
	void AddFishNet( float lonstart,float latstart,float lonend,float latend,float lonCellSize,float LatCellSize ,double qrz );
...
protected:
	...
	std::vector<std::string> m_basisLayerList;
