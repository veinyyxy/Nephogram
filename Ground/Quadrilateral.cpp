...
void Quadrilateral::SetOsgArray( osg::Vec3Array* coodinateArray )
{
	m_coodArray = coodinateArray;
}

void Quadrilateral::SetOsgColorArray( osg::Vec4Array &colorArray )
{
	*m_colorArray = colorArray;
}

void Quadrilateral::TransformGeometry()
{
	...
	if (pyramidVertices && !m_coodArray->empty())
	{
		...
		for (size_t i = 0; i< m_coodArray->size() ;i++)
		{
			if (GetBTransform())
			{
				m_pCoordinateTransform->Transform(m_coodArray->at(i),destVec3);
			}
			else
			{
...
#if 0
		nephogram_ground::Proj4Transform* pProj4 = dynamic_cast<nephogram_ground::Proj4Transform*>(m_pCorrdinateTransform);
		if(pProj4)
...
void Quadrilateral::operator=( const Quadrilateral& other )
{
...
Geometry* Quadrilateral::Clone()
{
	Quadrilateral* pNewGeom = new Quadrilateral();
	*pNewGeom = *this;
	return pNewGeom;
}

void Quadrilateral::CreateOsgFromGeos(osg::Node* pNode)
{
...
void Quadrilateral::SetCoordinateTransform( CoordinateTransform* pCT )
{
